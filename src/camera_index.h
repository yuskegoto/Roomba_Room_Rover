...
<!-- <div class="input-group hidden" id="led-group">
...
<!-- <a id="save-still" href="http://192.168.2.100/#" class="button save" download="capture.jpg">Save</a> -->
...
// function setXclk(xclk, cb){fetchUrl(`${baseHost}/xclk?xclk=${xclk}`, cb);}
...
// area.addEventListener("touchend", function (){enableWScommunication = false});

// area.onmousedown = function (evt){isMouseDown = true}
// area.onmouseup = function (evt){isMouseDown = false}
//area.onmousemove = function (evt) {
// area.onmousedown = function (evt) {
...
// let movemsg = `${baseHost}/move?_cb=${fromCenterX.toString()},${fromCenterY.toString()}`
// fetch(movemsg).then(response => {console.log(`request to ${movemsg} finished, status: ${response.status}`)})}
